...
@objcMembers public class BaseChatViewController: InputbarViewController,
    ...
    internal lazy var chatBackgroundView: PlaceholderView = {
        ...
        chatBackgroundView.setImage(UIImage(named: "gcc_chat_placeholder"))
    ...
    public override func viewDidLoad() {
        ...
        self.leftButton.setImage(UIImage(named:  "plus_icon"), for: .normal)
        self.leftButton.accessibilityLabel = NSLocalizedString("Share a file from your GCC Cloud", comment: "")
        ...
        self.tableView?.register(BaitshookMessageSeparatorTableViewCell.self, forCellReuseIdentifier: BaitshookMessageSeparatorTableViewCell.identifier)
    ...
    func showVoiceMessageRecordButton() {
        ...
        self.rightButton.setImage(UIImage(named: "voice_record"), for: .normal)
        //self.rightButton.setButtonStyle(style: .secondary)
    ...
    func addMenuToLeftButton() {
        ...
        let ncFilesAction = UIAction(title: filesAppName, image: UIImage(named: "gcc_talk")?.withRenderingMode(.alwaysTemplate)) { [unowned self] _ in
            self.textView.resignFirstResponder()
    ...
    func presentPollCreation() {
        let pollCreationVC = BaitshookPollCreationViewController(room: room)
    ...
    func prependMessages(historyMessages: [NCChatMessage], addingBlockSeparator shouldAddBlockSeparator: Bool) -> IndexPath? {
        ...
        if shouldAddBlockSeparator {
            ...
            blockSeparatorMessage.messageId = BaitshookMessageSeparatorTableViewCell.chatBlockSeparatorId
    ...
    public func tableView(_ tableView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {
        ...
        for indexPath in indexPaths {
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                guard message.messageId != BaitshookMessageSeparatorTableViewCell.unreadMessagesSeparatorId,
                      message.messageId != BaitshookMessageSeparatorTableViewCell.unreadMessagesWithSummarySeparatorId,
                      message.messageId != BaitshookMessageSeparatorTableViewCell.chatBlockSeparatorId
                else { return }
    ...
    func getCell(for message: NCChatMessage) -> UITableViewCell {
        if message.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesSeparatorId || message.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesWithSummarySeparatorId,
           let cell = self.tableView?.dequeueReusableCell(withIdentifier: BaitshookMessageSeparatorTableViewCell.identifier) as? BaitshookMessageSeparatorTableViewCell {

            cell.messageId = message.messageId
            cell.separatorLabel.text = BaitshookMessageSeparatorTableViewCell.unreadMessagesSeparatorText
            cell.delegate = self

            if message.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesWithSummarySeparatorId {
        ...
        if message.messageId == BaitshookMessageSeparatorTableViewCell.chatBlockSeparatorId,
           let cell = self.tableView?.dequeueReusableCell(withIdentifier: BaitshookMessageSeparatorTableViewCell.identifier) as? BaitshookMessageSeparatorTableViewCell {

            cell.messageId = message.messageId
            cell.separatorLabel.text = BaitshookMessageSeparatorTableViewCell.chatBlockSeparatorText
            return cell
        }
    ...
    func getCellHeight(for message: NCChatMessage, with originalWidth: CGFloat) -> CGFloat {
        ...
        // Chat separators
        if message.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesSeparatorId ||
            message.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesWithSummarySeparatorId ||
            message.messageId == BaitshookMessageSeparatorTableViewCell.chatBlockSeparatorId {
    ...
    public override func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        if message.isSystemMessage || message.isDeletedMessage ||
            message.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesSeparatorId ||
            message.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesWithSummarySeparatorId ||
            message.messageId == BaitshookMessageSeparatorTableViewCell.chatBlockSeparatorId {
    ...
    internal func indexPathForUnreadMessageSeparator() -> IndexPath? {
        return self.indexPathAndMessageFromEnd(with: {
            $0.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesSeparatorId || $0.messageId == BaitshookMessageSeparatorTableViewCell.unreadMessagesWithSummarySeparatorId
        })?.indexPath
    }
