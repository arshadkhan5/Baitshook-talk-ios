...
//    }
    ...
    override func viewDidLoad() {
        ...
        // App logo
        self.appLogoImageView.image = UIImage(named: "gcc_talk_logo")
      ...
      //  qrCodeButton.setTitle(NSLocalizedString("Scan QR code", comment: ""), for: .normal)
       // qrCodeButton.isHidden = !QRScannerViewController.isDataScannerSupported()
        ...
        // Buttons style
        let buttonColor = NCUtils.color(fromHexString: "#0B2A3A")
        [loginButton, /*qrCodeButton,*/ importAccountButton].forEach { button in
            button.backgroundColor = buttonColor//NCAppBranding.brandColor()
...
//            }
    ...
    func showServerNotFoundAlert(withError error: String?) {
        if isBrandedApp.boolValue {
            ...
            if let error {
                self.showAlert(
                    title: NSLocalizedString("GCC CLoud server not found", comment: ""),
                    message: String(format: "%@\n%@", error, NSLocalizedString("Please check that you entered the correct GCC Cloud server address.", comment: "")))

            } else {
                self.showAlert(
                    title: NSLocalizedString("GCC Cloud server not found", comment: ""),
                    message: NSLocalizedString("Please check that you entered the correct GCC Cloud  server address.", comment: ""))
            }
        }
    }
