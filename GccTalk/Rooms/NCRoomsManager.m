...
- (void)startCall:(BOOL)video inRoom:(NCRoom *)room withVideoEnabled:(BOOL)enabled asInitiator:(BOOL)initiator silently:(BOOL)silently recordingConsent:(BOOL)recordingConsent andVoiceChatMode:(BOOL)voiceChatMode
{
    if (!_callViewController) {
        TalkAccount *activeAccount = [[NCDatabaseManager sharedInstance] activeAccount];
        _callViewController = [[BaitshookCallViewController alloc] initFor:room asUser:activeAccount.userDisplayName audioOnly:!video];
...
- (void)callViewControllerWantsToBeDismissed:(BaitshookCallViewController *)viewController
{
...
- (void)callViewControllerWantsVideoCallUpgrade:(BaitshookCallViewController *)viewController
{
...
- (void)callViewController:(BaitshookCallViewController *)viewController wantsToSwitchFromRoom:(NSString *)from toRoom:(NSString *)to
{
    if (_callViewController == viewController) {
...
- (void)callViewControllerDidFinish:(BaitshookCallViewController *)viewController
{
    if (_callViewController == viewController) {
        NCRoom *room = _callViewController.room;
