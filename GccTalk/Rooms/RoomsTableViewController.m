...
- (void)createNewConversationButton
{
    if ([[NCSettingsController sharedInstance] canCreateGroupAndPublicRooms] ||
        ...
        UIImage *icon = [UIImage imageNamed:@"plus_icon"];

        _newConversationButton = [[UIBarButtonItem alloc]
            initWithImage:icon
...
#if defined(__IPHONE_26_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_26_0
        if (@available(iOS 26.0, *)) {
            _newConversationButton.tintColor = UIColor.whiteColor/*[NCAppBranding elementColor]*/;
            ...
            if ([UIDevice currentDevice].userInterfaceIdiom != UIUserInterfaceIdiomPhone) {
                ...
                _newConversationButton.image = [UIImage imageNamed:@"plus_icon"];
...
{
    if (filter == kRoomsFilterAll) {
        return [UIImage imageNamed:@"gcc_talk"];
    }
